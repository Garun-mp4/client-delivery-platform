...
- Активных задач реализации нет. Milestone 04 объединён с `main`.
...
1. Следующий milestone — 05 (анкеты и сбор информации), но не начинать без явного запроса.
2. Перед реализацией Milestone 05 создать отдельную ветку от актуального чистого `main`.
