...
4. `unsafe` URL нельзя опубликовать. `safe/reachable` публикуется обычным действием.
   ...
   Публикация public `safe/reachable` версии в той же транзакции создаёт одно идемпотентное задание
   project cover capture. Password-protected и недоступные версии renderer не открывает.
...
Автоснимок выполняется отдельным `ProjectCoverRenderer` adapter в worker. Chromium не получает
session cookies или preview password; каждый document/redirect/subresource перехватывается и
fulfill-ится тем же IP-pinned SSRF-safe transport. При retry/terminal failure предыдущая обложка
остаётся текущей. Периодического capture нет.
