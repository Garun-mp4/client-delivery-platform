...
### ADR-035. Прямая приватная загрузка через S3-compatible adapter

**Решение:** web создаёт tenant/project-scoped `FileObject`, резервирует quota и выдаёт короткий
presigned PUT. Length, MIME и checksum metadata входят в signature. После browser upload отдельный
completion endpoint выполняет HEAD и переводит запись в `uploaded/pending`; download всегда заново
авторизуется и выдаёт presigned GET на 60 секунд. Bucket приватный, CSP/CORS origins явные.

**Альтернативы:** проксировать до 100 MiB через Next.js/Vercel; public bucket со скрытыми URL; хранить
provider SDK непосредственно в business services; multipart upload уже в MVP.

**Причина:** web не держит большие body и остаётся совместимым с serverless, storage provider
заменяется без изменения domain logic, а один разработчик поддерживает простой одночастный pipeline.
Production-зависимости `@aws-sdk/client-s3` и `@aws-sdk/s3-request-presigner` нужны только
`packages/storage`; alternatives — MinIO SDK или ручная SigV4 — сильнее привязаны к provider либо
создают неоправданный security-sensitive код.

### ADR-036. Обязательный асинхронный quarantine pipeline

**Решение:** `initiated → uploaded/pending → scanning → available|rejected|failed`. До `available`
metadata и URL не выдаются. Worker повторно вычисляет SHA-256, sniff MIME и вызывает ClamAV TCP
adapter; временные ошибки повторяются до пяти раз с backoff, зависшие claims переобрабатываются.
Clean image получает WebP preview с auto-rotation без переноса EXIF. Перевод в available, завершение
revision/action, audit и outbox атомарны.

**Альтернативы:** доверять client MIME/checksum; synchronous scan в request; BullMQ до появления
других job types; публиковать файл при недоступном scanner; генерировать previews в браузере.

**Причина:** scanner/provider outage не раскрывает непроверенный контент, PostgreSQL `SKIP LOCKED`
достаточен для текущего объёма без новой queue-зависимости. Production-зависимость `sharp` обоснована
безопасным server-side decoding/resize и удалением metadata; ImageMagick subprocess сложнее
переносить и изолировать, browser preview нельзя считать доверенным.

### ADR-037. Append-only material revisions и quota reservation

**Решение:** material хранит current/final projection, но каждая передача создаёт новую
`MaterialRevision`; предыдущая принятая версия становится `replaced`, её FileLink сохраняется.
Workspace блокируется на время quota calculation, а initiated uploads уже занимают declared bytes.
Composite foreign keys и отдельный current-revision FK фиксируют tenant/project/material ownership.
File/image answers анкеты ссылаются на тот же FileObject и проверяются при submit.

**Альтернативы:** перезаписывать один object key; считать quota только по available files; доверять
file UUID из ответа анкеты; создавать отдельную файловую систему для анкет.

**Причина:** retries не превышают quota, история проверяема, а единый security pipeline уменьшает
число способов обойти quarantine и tenant isolation.

### ADR-038. Role-based application shell и маршрут проекта до Milestone 07

**Решение:** между Milestones 06 и 07 добавлен Milestone 06.5. Workspace получает role-based
overview/navigation, а внутри проекта используется отдельная контекстная навигация. Повторяемый
`ProjectRoute` показывает состояние, ответственную сторону, следующее действие и ожидаемый результат.
Design tokens и patterns фиксируются в `.interface-design/system.md`; подробные UX-ограничения — в
`docs/UX_FOUNDATION.md`.

**Альтернативы:** завершить все MVP-модули и провести один большой redesign; косметически обновить
цвета без изменения информационной архитектуры; показывать владельцу и клиенту одинаковую sidebar.

**Причина:** поздний redesign потребовал бы одновременно менять Milestones 01–10 и закрепил бы
неудобную навигацию в новых модулях. Role-based shell следует исходному принципу «одно главное
действие», уменьшает cognitive load клиента и не меняет server authorization.

### ADR-039. Performance UX измеряется на production build

**Решение:** cold compilation Next.js development режима документируется отдельно. Acceptance
основывается на production build и повторной навигации; каждое действие всё равно обязано немедленно
показать pending feedback. Оптимизируются только измеренные queries/render paths, без speculative
cache чувствительных tenant data.

**Альтернативы:** считать все development delays дефектами production; скрыть задержки только
анимацией; добавить общий client cache до определения security boundaries.

**Причина:** корректное измерение отделяет стоимость компиляции от серверных/DB проблем, а
немедленная обратная связь сохраняет ощущение контроля даже для обоснованно асинхронных операций.

### ADR-040. Две оси URL-проверки и append-only review model

**Решение:** `SiteVersion` неизменяема по URL и публикуется клиенту только после асинхронной
SSRF-safe проверки. Security и availability хранятся отдельно. Worker проверяет все DNS-адреса и
каждый redirect, блокирует специальные IP ranges и закрепляет соединение за проверенным IP.
Security result старше 10 минут перед публикацией инвалидируется. `FeedbackItem` владеет workflow,
а `Comment` остаётся сообщением с visibility/edit marker/tombstone. Preview-пароль шифруется и
раскрывается только после повторной server policy.

**Альтернативы:** синхронно fetch URL из web request; считать недоступный password-protected preview
опасным; разрешить ручной override unsafe URL; менять URL существующей версии; использовать
комментарий как неявное изменение статуса.

**Причина:** разделение security/availability поддерживает реальные закрытые preview без SSRF
исключений, append-only версии дают проверяемую историю, а отдельный workflow замечаний исключает
неоднозначные переходы из обычной переписки.

### ADR-041. Отдельный лимит внутреннего создания session после invitation

**Решение:** публичный magic-link остаётся ограничен пятью запросами в минуту. После успешной
транзакционной проверки одноразового invitation внутренний Better Auth flow создания session
использует отдельный лимит 20 в минуту. Публичный invitation endpoint дополнительно сохраняет
собственный rate limit 20 за 5 минут и одноразовый hashed token.

**Альтернативы:** ослабить общий публичный magic-link limit; создать самописную session/cookie;
оставлять принятого пользователя без session при конкуренции лимитов.

**Причина:** внутренний вызов не является новым пользовательским запросом magic-link. Раздельный
лимит сохраняет защиту публичного endpoint и не заставляет пользователя повторно входить после
валидного приглашения.

### ADR-042. Security patch runtime-зависимостей и ограниченное исключение dev advisory

**Решение:** после объединения Milestones 06–07 Next.js обновлён до `16.2.11`, а `sharp` закреплён
на `0.35.3`, включая транзитивное использование Next.js. Advisory
`GHSA-mh99-v99m-4gvg` для `brace-expansion` временно исключён из `pnpm audit`: уязвимая версия
достижима только через ESLint/minimatch в локальном lint toolchain и не входит в web/worker
production artifact. Исключение ограничено одним GHSA и должно быть удалено после совместимого
обновления ESLint-цепочки.

**Альтернативы:** глобально подменить `brace-expansion` на `5.0.8`; перейти на ESLint 10 немедленно;
оставить CI красным; отключить audit целиком.

**Причина:** глобальный override несовместим с `minimatch@3` и ломает lint, а ESLint 10 пока
...
### ADR-049. Бесплатный hybrid staging для внутренней проверки

**Решение:** до утверждения бюджета используется изолированный no-cost staging: web — Vercel
Hobby, PostgreSQL и private S3-compatible storage — Supabase Free, Redis — Upstash Free. Worker,
Mailpit и ClamAV остаются локальными и подключаются к внешним зависимостям через существующие
адаптеры. Это внутренняя pilot/rehearsal среда, а не production и не полноценный клиентский пилот:
background jobs и письма выполняются только пока компьютер разработчика включён. Credentials
хранятся только в provider secret stores или процессе запуска и не попадают в Git. Для PostgreSQL
поддерживается `DATABASE_SSL_CA`: pool удаляет конфликтующие URL SSL-параметры и проверяет сервер
по явно настроенному PEM CA. Миграции выполняются отдельным release step и сохраняют стандартный
journal Drizzle.

Ограничения бесплатного тарифа фиксируются конфигурацией среды, не меняя продуктовые defaults:
до 50 000 000 bytes на файл и 1 000 000 000 bytes на workspace. В staging не привязываются карты,
не покупаются домен, email provider, always-on worker, scanner или monitoring. Переход к внешнему
клиентскому пилоту требует отдельного решения и замены локальных worker/email/scanner на постоянно
доступные сервисы.

**Альтернативы:** преждевременно купить Railway/R2/Resend; назвать локальный Compose production;
запустить web без worker/scanner; ослабить TLS через `rejectUnauthorized: false`.

**Причина:** конфигурация даёт реальный публичный web и внешние данные без расходов, сохраняет
изоляцию окружения и проверяемый TLS, но честно ограничивает доступность фоновых функций.

### ADR-050. Локальный readiness gate и единый capture eligibility

**Решение:** перед Milestone 11 выполняется отдельный Milestone 10.5 в полном локальном Compose.
Готовность к созданию снимка вычисляется в server-side policy/service и имеет только семь
allowlisted результатов: `eligible`, `no_version`, `check_pending`, `unsafe`, `unreachable`,
`password_protected`, `not_published`. UI и Route Handler не повторяют это бизнес-правило и не
раскрывают URL, внутренние ID, retries или worker diagnostics. Для ручного capture одновременно
допускается не более одного активного задания выбранной версии; автоматическое задание публикации
остаётся идемпотентным. Readiness 10.5 разрешает последующую локальную разработку, но не означает
production readiness, внешний pilot или разрешение создавать платную инфраструктуру.

Milestone 11 заранее делится на 11.1 templates, 11.2 Change Requests, 11.3 manual payments,
11.4 search/export/quotas и 11.5 Telegram/white-label/analytics. Это только порядок будущей работы;
каждая часть требует отдельного решения о старте.

**Альтернативы:** вычислять причину недоступности в React; возвращать клиенту сырые статусы/ID;
считать любой `clientVisible` URL пригодным; начинать весь Milestone 11 одним большим изменением.

**Причина:** единый серверный контракт устраняет расхождение UX и безопасности, а локальный
rehearsal проверяет реальный MVP без затрат и без преждевременного расширения scope.

**Уточнение реализации:** если DNS возвращает только разрешённые публичные адреса обоих семейств,
transport предпочитает IPv4, чтобы отсутствие IPv6 route не превращало доступный сайт в
`unreachable`; наличие хотя бы одного private/special ответа по-прежнему блокирует весь запрос.
Playwright поднимает worker artifact на отдельном порту `3101` с запрещённым process reuse, чтобы
E2E не мог незаметно использовать старый Compose-worker на `3001`.
...
До отдельного решения остаются открытыми точный домен, юрисдикция/регион данных, production
accounts/тарифы, sender domain, конкретный deployment scanner, production retention для incomplete
uploads и финальный юридический текст. Для local/dev в Milestone 06 принят конфигурируемый срок
очистки incomplete uploads 24 часа. Открытые production-решения не разрешают создавать реальные
ресурсы или secrets и не блокируют локальную разработку.
