...
import { createClientCompany, parseClientCompanyInput } from '@garun/core/projects';
...
export async function POST(request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const created = await createClientCompany(database, tenant, input, {
      requestId: request.headers.get('x-request-id') ?? undefined,
    });
    return NextResponse.redirect(
      publicAppUrl(
        environment.PUBLIC_APP_URL,
        `/workspace/${slug}/clients/${created.id}?success=created`,
      ),
      303,
    );
  } catch {
    return NextResponse.redirect(
      publicAppUrl(environment.PUBLIC_APP_URL, `/workspace/${slug}/clients?error=create`),
      303,
    );
  }
}
