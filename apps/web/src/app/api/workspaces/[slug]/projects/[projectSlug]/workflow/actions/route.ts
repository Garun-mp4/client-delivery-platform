...
import { createAction, parseActionInput } from '@garun/core/workflow';
...
export async function POST(
  ...
  try {
    await createAction(
      ...
      tenant,
      projectSlug,
      parseActionInput(formRecord(await request.formData())),
      { requestId: request.headers.get('x-request-id') ?? undefined },
    );
    return NextResponse.redirect(
      publicAppUrl(
        environment.PUBLIC_APP_URL,
        `/workspace/${slug}/projects/${projectSlug}/workflow?success=action-created`,
      ),
      303,
    );
  } catch {
    return NextResponse.redirect(
      publicAppUrl(
        environment.PUBLIC_APP_URL,
        `/workspace/${slug}/projects/${projectSlug}/workflow?error=action`,
      ),
      303,
    );
  }
