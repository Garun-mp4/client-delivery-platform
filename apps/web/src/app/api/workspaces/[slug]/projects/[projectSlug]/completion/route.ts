...
import { completeProject } from '@garun/core/projects';
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.redirect(
      publicAppUrl(environment.PUBLIC_APP_URL, `/workspace/${slug}/projects/${projectSlug}`),
      303,
    );
  } catch {
    return NextResponse.redirect(
      publicAppUrl(
        ...
        `/workspace/${slug}/projects/${projectSlug}/completion?error=gate`,
      ),
      303,
    );
  }
