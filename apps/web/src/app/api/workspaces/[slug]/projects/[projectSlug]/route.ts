...
import { parseProjectInput, updateProject } from '@garun/core/projects';
...
export async function POST(
  ...
  try {
    ...
    const updated = await updateProject(database, tenant, projectSlug, input, {
      requestId: request.headers.get('x-request-id') ?? undefined,
    });
    return NextResponse.redirect(
      publicAppUrl(
        environment.PUBLIC_APP_URL,
        `/workspace/${slug}/projects/${updated.slug}?success=updated`,
      ),
      303,
    );
  } catch {
    return NextResponse.redirect(
      publicAppUrl(
        environment.PUBLIC_APP_URL,
        `/workspace/${slug}/projects/${projectSlug}?error=update`,
      ),
      303,
    );
  }
