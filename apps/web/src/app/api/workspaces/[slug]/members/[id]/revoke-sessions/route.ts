...
export async function POST(
  ...
  if (!tenant || !can(tenant, 'sessions.revoke.other'))
    return NextResponse.json({ error: { code: 'NOT_FOUND' } }, { status: 404 });
  const form = await request.formData();
  if (form.get('confirm') !== 'yes')
    return NextResponse.json({ error: { code: 'CONFIRMATION_REQUIRED' } }, { status: 400 });
  const [target] = await database.db
    .select({ userId: workspaceMembership.userId, role: workspaceMembership.role })
    .from(workspaceMembership)
    .where(
      and(eq(workspaceMembership.id, id), eq(workspaceMembership.workspaceId, tenant.workspaceId)),
    )
    .limit(1);
  ...
  await database.db.transaction(async (tx) => {
    ...
    await tx.insert(auditEvent).values({
      ...
      action: 'session.revoked_by_owner',
      entityType: 'user',
  ...
  return NextResponse.redirect(new URL(`/workspace/${slug}?success=sessions`, request.url), 303);
}
